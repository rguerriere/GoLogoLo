...
import djf.ui.controllers.AppViewportController;
import java.net.URL;
 ...
 * custom provided for each application.
 *
...
public class AppGUIModule {
    ...
    protected AppViewportController viewportController;
    ...
    // THIS IS FOR THE VIEWPORT BUTTONS IF WE'RE USING THEM
    protected ToolBar viewportToolbar;
     ...
     * ready for use until all language settings have been loaded and then
     * loadGUI is called.
     *
    ...
    public void loadGUI(Stage initPrimaryStage) throws LanguageException {
        ...
        // WITHOUT THE TOP TOOLBAR
            ...
            props.addProperty(HAS_VIEWPORT_TOOLBAR, FALSE);
        }
    ...
    public AppViewportController getViewportController() {
    ...
    public ToolBar getViewportToolbar() {
    ...
    private void initTopToolbar(AppTemplate app) {
        ...
        if (props.isTrue(HAS_VIEWPORT_TOOLBAR)) {
            initViewportToolbar();
        }
    ...
    private void initViewportToolbar() {
        viewportToolbar = new ToolBar();
        topToolbarPane.getChildren().add(viewportToolbar);
        viewportController = new AppViewportController(app);
        PropertiesManager props = PropertiesManager.getPropertiesManager();
        if (props.isTrue(HAS_HOME)) {
            Button homeButton = nodesBuilder.buildIconButton(HOME_BUTTON, null, viewportToolbar, CLASS_DJF_ICON_BUTTON, HAS_KEY_HANDLER, FOCUS_TRAVERSABLE, ENABLED);
            homeButton.setOnAction(e -> {
                viewportController.processResetViewPort();
            });
        }
        if (props.isTrue(HAS_ZOOM_IN)) {
            Button zoomInButton = nodesBuilder.buildIconButton(ZOOM_IN_BUTTON, null, viewportToolbar, CLASS_DJF_ICON_BUTTON, HAS_KEY_HANDLER, FOCUS_TRAVERSABLE, ENABLED);
            zoomInButton.setOnAction(e -> {
                viewportController.processZoomInViewPort();
            });
        }
        if (props.isTrue(HAS_ZOOM_OUT)) {
            Button zoomOutButton = nodesBuilder.buildIconButton(ZOOM_OUT_BUTTON, null, viewportToolbar, CLASS_DJF_ICON_BUTTON, HAS_KEY_HANDLER, FOCUS_TRAVERSABLE, ENABLED);
            zoomOutButton.setOnAction(e -> {
                viewportController.processZoomOutViewPort();
            });
        }
        if (props.isTrue(HAS_RESIZE)) {
            Button resizeButton = nodesBuilder.buildIconButton(RESIZE_BUTTON, null, viewportToolbar, CLASS_DJF_ICON_BUTTON, HAS_KEY_HANDLER, FOCUS_TRAVERSABLE, ENABLED);
            resizeButton.setOnAction(e -> {
                viewportController.processResizeViewPort();
            });
        }
        if (props.isTrue(HAS_SNAP)) {
            CheckBox Snap = nodesBuilder.buildCheckBox(SNAP_CHECKBOX, null, viewportToolbar, CLASS_EMPTY, HAS_KEY_HANDLER, FOCUS_TRAVERSABLE, ENABLED);
            Snap.selectedProperty().addListener(e -> {
                viewportController.processSnapViewPort();
            });
        }
    }
    ...
    public void updateToolbar(boolean b){};
}
