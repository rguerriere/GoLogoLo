...
public class AppWelcomeDialog extends Stage {
    ...
    public AppWelcomeDialog(AppTemplate app) {
        ...
        createNewButton.setOnAction(e->{
            ...
            Pane CanvasHolder = (Pane)((BorderPane)app.getWorkspaceComponent().getWorkspace()).getCenter();
            CanvasHolder.setClip(outputClip);
            outputClip.setWidth(CanvasHolder.getWidth());
            outputClip.setHeight(CanvasHolder.getHeight());
            Pane Canvas = (Pane) app.getGUIModule().getGUINode(GOLO_CANVAS_PANE);
            Canvas.setPrefSize(CanvasHolder.getWidth(), CanvasHolder.getHeight());
        });
        newPane = new HBox();
